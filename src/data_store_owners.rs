...
use crate::store_data::{Owner, MAX_OWNERS};

const fn owner(name: &'static str, max_cost: i16, max_inflate: u8, min_inflate: u8, haggles_per: u8, race: u8, max_insults: u8) -> Owner {
    Owner {
        name,
        max_cost,
        max_inflate,
...
pub static STORE_OWNERS: [Owner; MAX_OWNERS] = [
    owner("Erick the Honest       (Human)      General Store", 250, 175, 108, 4, 0, 12),
    owner("Mauglin the Grumpy     (Dwarf)      Armory", 32000, 200, 112, 4, 5, 5),
    owner("Arndal Beast-Slayer    (Half-Elf)   Weaponsmith", 10000, 185, 110, 5, 1, 8),
    owner("Hardblow the Humble    (Human)      Temple", 3500, 175, 109, 6, 0, 15),
    owner("Ga-nat the Greedy      (Gnome)      Alchemist", 12000, 220, 115, 4, 4, 9),
    owner("Valeria Starshine      (Elf)        Magic Shop", 32000, 175, 110, 5, 2, 11),
    owner("Andy the Friendly      (Halfling)   General Store", 200, 170, 108, 5, 3, 15),
    owner("Darg-Low the Grim      (Human)      Armory", 10000, 190, 111, 4, 0, 9),
    owner("Oglign Dragon-Slayer   (Dwarf)      Weaponsmith", 32000, 195, 112, 4, 5, 8),
    owner("Gunnar the Paladin     (Human)      Temple", 5000, 185, 110, 5, 0, 23),
    owner("Mauser the Chemist     (Half-Elf)   Alchemist", 10000, 190, 111, 5, 1, 8),
    owner("Gopher the Great!      (Gnome)      Magic Shop", 20000, 215, 113, 6, 4, 10),
    owner("Lyar-el the Comely     (Elf)        General Store", 300, 165, 107, 6, 2, 18),
    owner("Mauglim the Horrible   (Half-Orc)   Armory", 3000, 200, 113, 5, 6, 9),
    owner("Ithyl-Mak the Beastly  (Half-Troll) Weaponsmith", 3000, 210, 115, 6, 7, 8),
    owner("Delilah the Pure       (Half-Elf)   Temple", 25000, 180, 107, 6, 1, 20),
    owner("Wizzle the Chaotic     (Halfling)   Alchemist", 10000, 190, 110, 6, 3, 8),
    owner("Inglorian the Mage     (Human?)     Magic Shop", 32000, 200, 110, 7, 0, 10),
];

pub static SPEECH_SALE_ACCEPTED: [&str; 14] = [
    "Done!",
    "Accepted!",
    "Fine.",
    "Agreed!",
...
pub static SPEECH_BUYING_HAGGLE_FINAL: [&str; 3] = [
    "I'll pay no more than %A1; take it or leave it.",
    "You'll get no more than %A1 from me.",
    "%A1 and that's final.",
];

pub static SPEECH_BUYING_HAGGLE: [&str; 15] = [
...
pub static SPEECH_INSULTED_HAGGLING_DONE: [&str; 5] = [
...
pub static SPEECH_GET_OUT_OF_MY_STORE: [&str; 5] = [
    "Out of my place!",
    "out... Out... OUT!!!",
    "Come back tomorrow.",
...
pub static SPEECH_HAGGLING_TRY_AGAIN: [&str; 10] = [
